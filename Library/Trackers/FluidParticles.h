...
public:
	...
	// surface.
	FluidParticles(Real particleRadius, int countPerCell, Real oversample = 1., bool trackVelocity = false)
	...
	void drawPoints(Renderer& renderer, const Vec3f& colour = Vec3f(1,0,0), Real pointSize = 1) const;
	...
	// to the count.
	void reseed(const LevelSet& surface, Real minDensity = 1., Real maxDensity = 1., const VectorGrid<Real>* velocity = nullptr, Real seed = 0);

	// Push particles inside collision objects back to the surface
	void bumpParticles(const LevelSet& collision);

	int particleCount() const { return myParticles.size(); }
	
	Vec2R getPosition(int particleIndex) const
	{
