...
namespace GeometricMultigridOperators
{
...

	{
		assert(solution.size() == rhs.size() &
				solution.size() == domainCellLabels.size());

		UniformGrid<StoreReal> tempSolution = solution;

		const SolveReal gridScalar = 1. / Util::sqr(SolveReal(dx));
										...
										const VectorGrid<StoreReal> *boundaryWeights)
	{
		...
		tbb::parallel_for(tbb::blocked_range<int>(0, boundaryCells.size(), tbbGrainSize), [&](const tbb::blocked_range<int> &range)
		{
							...
							const VectorGrid<StoreReal> *boundaryWeights)
	{
		assert(destination.size() == source.size() &&
				source.size() == domainCellLabels.size());
		...
		tbb::parallel_for(tbb::blocked_range<int>(0, totalVoxels, tbbGrainSize), [&](const tbb::blocked_range<int> &range)
		{
			for (int flatIndex = range.begin(); flatIndex != range.end(); ++flatIndex)
			{
					...
					destinationCellLabels(cell) == CellLabels::BOUNDARY_CELL)
				{
					...
					Vec<SolveReal, 2> samplePoint = .5 * (Vec<SolveReal, 2>(cell) + Vec<SolveReal, 2>(.5)) - Vec<SolveReal, 2>(.5);

					Vec2i startCell = Vec2i(samplePoint);

					Vec<SolveReal, 2> interpWeight = samplePoint - Vec<SolveReal, 2>(startCell);
					...
					destinationGrid(cell) += Util::bilerp(values[0][0], values[1][0], values[0][1], values[1][1], interpWeight[0], interpWeight[1]);
				}
			}
		});
	}
	...
	SolveReal dotProduct(const UniformGrid<StoreReal> &vectorGridA,
							...
							const UniformGrid<CellLabels> &domainCellLabels)
	{
		assert(vectorGridA.size() == vectorGridB.size() &&
				vectorGridB.size() == domainCellLabels.size());

		using ParallelReal = tbb::enumerable_thread_specific<SolveReal>;

		ParallelReal parallelReal(SolveReal(0));
		...
		const UniformGrid<CellLabels> &domainCellLabels)
	{
		...
		tbb::parallel_for(tbb::blocked_range<int>(0, totalVoxels, tbbGrainSize), [&](const tbb::blocked_range<int> &range)
		{
			...
			for (int flatIndex = range.begin(); flatIndex != range.end(); ++flatIndex)
			{
				...
				if (domainCellLabels(cell) == CellLabels::INTERIOR_CELL ||
					domainCellLabels(cell) == CellLabels::BOUNDARY_CELL)
					localSqrNorm += SolveReal(vectorGrid(cell)) * SolveReal(vectorGrid(cell));
			}
		});
							...
							const UniformGrid<CellLabels> &domainCellLabels)
	{
		...
		tbb::parallel_for(tbb::blocked_range<int>(0, totalVoxels, tbbGrainSize), [&](const tbb::blocked_range<int> &range)
		{
			...
			for (int flatIndex = range.begin(); flatIndex != range.end(); ++flatIndex)
			{
				...
				if (domainCellLabels(cell) == CellLabels::INTERIOR_CELL ||
					domainCellLabels(cell) == CellLabels::BOUNDARY_CELL)
					localMaxError = std::max(fabs(SolveReal(vectorGrid(cell))), localMaxError);
			}
		});
									...
									const VectorGrid<StoreReal> &boundaryWeights)
	{
		...
		using ParallelBoundaryCellList = tbb::enumerable_thread_specific<std::vector<Vec2i>>;

		ParallelBoundaryCellList parallelBoundaryCellList;
		...
		int listSize = 0;
		parallelBoundaryCellList.combine_each([&](const std::vector<Vec2i> &localBoundaryCellList)
		{
			listSize += localBoundaryCellList.size();
		});

		boundaryCellList.reserve(listSize);

		parallelBoundaryCellList.combine_each([&](const std::vector<Vec2i> &localBoundaryCellList)
		{
			boundaryCellList.insert(boundaryCellList.end(), localBoundaryCellList.begin(), localBoundaryCellList.end());
		});

		tbb::parallel_for(tbb::blocked_range<int>(0, boundaryCellList.size(), tbbGrainSize), [&](const tbb::blocked_range<int> &range)
		{
			for (int cellIndex = range.begin(); cellIndex != range.end(); ++cellIndex)
			{
										...
										const int axis)
	{
		...
		int totalVoxels = baseBoundaryWeights.size(axis)[0] * baseBoundaryWeights.size(axis)[1];

		tbb::parallel_for(tbb::blocked_range<int>(0, totalVoxels, tbbGrainSize), [&](const tbb::blocked_range<int> &range)
		{
			for (int flatIndex = range.begin(); flatIndex != range.end(); ++flatIndex)
			{
				Vec2i face = baseBoundaryWeights.grid(axis).unflatten(flatIndex);
