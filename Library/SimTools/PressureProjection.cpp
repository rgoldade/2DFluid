...
{
	assert(ghostFluidWeights.isGridMatched(cutCellWeights) && ghostFluidWeights.isGridMatched(myFluidVelocity));
			...
			for (unsigned axis : {0, 1})
				for (unsigned direction : {0, 1})
				{
					...
					if (weight > 0)
					{
						...
						int adjacentFluidIndex = myFluidCellIndex(Vec2ui(adjacentCell));
...
{
	assert(liquidWeights.isGridMatched(myFluidVelocity));
