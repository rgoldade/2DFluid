...
public:
		...
		, m_full_nx(nx)
	{
		...
		case XFACE:
			...
			this->resize(nx + Vec2st(1, 0), val);
			break;
		case YFACE:
			m_cell_offset = Vec2R(.5, .0);
			this->resize(nx + Vec2st(0, 1), val);
			break;
		case NODE:
			m_cell_offset = Vec2R(.0);
			this->resize(nx + Vec2st(1), val);
		}
	}
	...
	T maxval() const { auto result = std::max_element(this->m_grid.begin(), this->m_grid.end()); return *result; }
	T minval() const { auto result = std::min_element(this->m_grid.begin(), this->m_grid.end()); return *result; }
	...
	void draw_grid_cell(Renderer& renderer, const Vec2st& coord, const Vec3f& colour = Vec3f(0)) const;
...
T ScalarGrid<T>::interp(const Vec2R& pos, bool idx_space) const
{
	...
	case ZERO:
		if ((ipos[0] < 0.) || (ipos[1] < 0.) ||
			(ipos[0] > (Real)(this->m_nx[0] - 1)) ||
			(ipos[1] > (Real)(this->m_nx[1] - 1))) return T(0.);
	case CLAMP:
		ipos[0] = (ipos[0] < 0.) ? 0. : ((ipos[0] > (Real)(this->m_nx[0] - 1)) ? (Real)(this->m_nx[0] - 1) : ipos[0]);
		ipos[1] = (ipos[1] < 0.) ? 0. : ((ipos[1] > (Real)(this->m_nx[1] - 1)) ? (Real)(this->m_nx[1] - 1) : ipos[1]);
		break;
	// Useful debug check. Equivalent to "NONE" for release mode
	case ASSERT:
		assert((ipos[0] >= 0.) && (ipos[1] >= 0.) &&
					(ipos[0] <= (Real)(this->m_nx[0] - 1)) &&
					(ipos[1] <= (Real)(this->m_nx[1] - 1)));
		break;
	}
...
template<typename T>
void ScalarGrid<T>::draw_grid_cell(Renderer& renderer, const Vec2st& coord, const Vec3f& colour) const
{
