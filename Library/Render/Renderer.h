...
public:
  ...
  void
  setUserKeyboard(std::function<void(unsigned char, int, int)> keyFunction);
  void setUserMouseDrag(std::function<void(int, int)> dragFunction);
  void setUserDisplay(std::function<void()> displayFunction);

  void addPoint(const Vec2R &point, const Vec3f &colour = Vec3f(0, 0, 0),
                Real size = 1.);
  void addPoints(const std::vector<Vec2R> &points,
                 const Vec3f &colour = Vec3f(0, 0, 0), Real size = 1.);

  void addLine(const Vec2R &start, const Vec2R &end,
               const Vec3f &colour = Vec3f(0, 0, 0), const Real width = 1);
  void addLines(const std::vector<Vec2R> &start, const std::vector<Vec2R> &end,
                const Vec3f &colour, const Real width = 1);
  void addTris(const std::vector<Vec2R> &verts, const std::vector<Vec3i> &faces,
               const std::vector<Vec3f> &colour);
  void addQuads(const std::vector<Vec2R> &verts,
                const std::vector<Vec4i> &faces,
                const std::vector<Vec3f> &colours);
