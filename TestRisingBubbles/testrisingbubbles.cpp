...
void display()
{
	if (g_run || g_single_run)
	{
		...
		while (frame_time < g_dt)
		{
			...
			//g_sim->set_surface_tension(0.1);
			
			//g_sim->set_viscosity(.1);
...
int main(int argc, char** argv)
{
	...
	Mesh2D solid_mesh = square_mesh(center, Vec2R(1,2));
	solid_mesh.reverse();
	...
	Mesh2D waterside = square_mesh(center - Vec2R(0, .5), Vec2R(1,1.5));
	Mesh2D bubble = square_mesh(center - Vec2R(0, 1.5), Vec2R(.35));
	bubble.reverse();

	waterside.insert_mesh(bubble);
